...
vgg_name = ["VGG11", "VGG13", "VGG16", "VGG19"]


def get_model(vgg_name):
    model = None
    if vgg_name == "VGG11":
    ...
    model.classifier[6] = torch.nn.Linear(in_features=4096, out_features=512)
...
if __name__ == "__main__":
    ...
    est = Estimator(save_dir=Path.cwd(), device=device)
    ...
    # threshold = (
    #     neg_scores[far_threshold]
    #     if far_threshold < len(neg_scores)
    #     else neg_scores[-1]
    # )
    ...
    # print(
    #     f"验证集准确率: {100.0 * correct / total:.2f}%, Rank-1 Accuracy: {rank1_acc:.2f}%, TAR@FAR=1e-6: {tar:.2f}%"
    # )
